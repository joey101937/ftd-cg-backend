...
import { canCardBePlayedToZone, canPlayerAffordCard, getResourcesForTurn } from "./gameLogicService";
...
const attackEnemyBase = async (game, actionBody, playerId) => {
    ...
    const enemyHasBlockers = enemyVehicles.find(x => x.meta.keywords?.includes(VEHICLE_KEYWORDS.BLOCKER));
...
const playCardToZoneHandler = async (game, actionBody, playerId) => {
    ...
    const materialCost = playingCard.meta?.keywords?.includes(VEHICLE_KEYWORDS.HALF_COST) ? playingCard.materialCost/2 : playingCard.materialCost;
